...
public class BrickSpawner : MonoBehaviour
{
    ...
    public void Init()
    {
        foreach (BrickGridData data in brickList)
        {
            if (data.brick != null)
            {
                ObjectPool.instance.Despawn(data.brick.gameObject);
            }
        }
        brickList.Clear();
    ...
    {
        GameObject brickObj = ObjectPool.instance.Spawn("Brick");
        if (brickObj == null) return;
        ...
        while (true)
        {
            for (int i = 0; i < brickList.Count; i++)
            {
                ...
                if (data.IsCollected && Random.value > 0.5f && !IsPositionOccupied(data.position))
                {
                    ...
                    if (brickObj != null)
                    {
                        brickObj.transform.position = data.position;
                        brickObj.transform.rotation = Quaternion.identity;

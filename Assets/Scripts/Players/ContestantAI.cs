...
public class ContestantAI : Character
{
    ...
    private bool hasChosenFirstBridge = false;
    ...
    private void IdleState()
    {
        ...
        {
            currentBridge = GameManager.Instance.FindBestBridge(Color, CurrentFloor, currentBridge, !hasChosenFirstBridge);

            if (currentBridge != null)
            {
                targetPosition = currentBridge.TopPosition;
                SetState(State.Move);
                hasChosenFirstBridge = true;
            }
            return;
        }
